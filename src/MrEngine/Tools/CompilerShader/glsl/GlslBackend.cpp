...
#include "hlslcc_lib/glsl_parser_extras.h"

__pragma(warning(pop))

#include "hlslcc_lib/hash_table.h"
#include "hlslcc_lib/ir_rvalue_visitor.h"
#include "hlslcc_lib/PackUniformBuffers.h"
#include "hlslcc_lib/IRDump.h"
//@todo-rco: Remove STL!
#include <sstream>
#include "hlslcc_lib/hash_table.h"
...
class ir_gen_glsl_visitor : public ir_visitor
{
	...
	/** Whether the shader being cross compiled needs EXT_shadow_samplers. */
	bool bUsesES2ShadowSamplers;
	...
	*/
	...
	virtual void visit(ir_texture* tex)
	{
		...
		{
			if (!bIsES || !tex->sampler->type->sampler_shadow)
			{
		...
		{
			// gles2 阴影
			ralloc_asprintf_append(buffer, "shadow2DEXT(");
			bUsesES2ShadowSamplers = true;
		}
		else
		{
		...
		if (!bIsES || !tex->sampler->type->sampler_shadow)	// gles2阴影不需要第三个参数
		{
			// Emit LOD.
			if (op == ir_txl ||
	...
	void print_extern_vars(_mesa_glsl_parse_state* State, exec_list* extern_vars)
	{
		...
		foreach_iter(exec_list_iterator, iter, *extern_vars)
		{
			...
			const char* basetype = type->base_type == GLSL_TYPE_STRUCT ? type->name : type_str[type->base_type];
			if (type->base_type == GLSL_TYPE_SAMPLER && type->sampler_dimensionality == GLSL_SAMPLER_DIM_CUBE) // cube map
			{
				basetype = "c";
			}
	...
	void print_signature(_mesa_glsl_parse_state* state, bool bGroupFlattenedUBs)
	{
		...
		if (state->num_uniform_blocks > 0 && !state->bFlattenUniformBuffers)
		{
			...
			for (unsigned i = 0; i < state->num_uniform_blocks; i++)
			{
				...
				if (hash_table_find(used_uniform_blocks, block->name))
				{
					ralloc_asprintf_append(buffer, "%s%s(%d)%s",
						first ? "// @UniformBlocks: " : ",",
						block->name,
						Index,
						block->num_vars > 0 ? "{" : "");
					...
					for (int i = 0; i < block->num_vars; ++i)
					{
						...
						// const buffer 导出结构体的详细信息
						if (type->base_type == GLSL_TYPE_STRUCT)
						{
							bool bcomma = false;
							for (int structIdx = 0; structIdx < type->length; ++structIdx)
							{
								const glsl_type* sub_type = type->fields.structure[structIdx].type;
								check(sub_type);

								bool is_sub_array = sub_type->is_array();
								int array_sub_size = is_sub_array ? sub_type->length : 0;
								if (is_sub_array)
								{
									sub_type = sub_type->fields.array;
								}
								ralloc_asprintf_append(buffer, "%s%s%s%s",
									bcomma ? "|" : "#",
									type_str[sub_type->base_type],
									col_str[sub_type->matrix_columns],
									row_str[sub_type->vector_elements]);
								if (is_sub_array)
								{
									ralloc_asprintf_append(buffer, "[%u]", array_sub_size);
								}
								ralloc_asprintf_append(buffer, ";%d:%s", -1, type->fields.structure[structIdx].name);
								bcomma = true;
							}
						}

						ralloc_asprintf_append(buffer, "%s", i == block->num_vars - 1 ? "}" : "");
	...
	void print_extensions(_mesa_glsl_parse_state* state, bool bUsesFramebufferFetchES2, bool bUsesDepthbufferFetchES2, bool bInUsesExternalTexture)
	{
		...
		if (bUsesES2ShadowSamplers)
		{
			ralloc_asprintf_append(buffer, "#ifndef DONTEMITEXTENSIONSHADERSHADOWSAMPLERSENABLE\n");
			ralloc_asprintf_append(buffer, "#extension GL_EXT_shadow_samplers : enable\n");
			ralloc_asprintf_append(buffer, "#endif\n");
		}
		...
		{
			ralloc_asprintf_append(buffer, "#ifdef VENUS_EXT_draw_instanced\n");
		...
		{
			ralloc_asprintf_append(buffer, "\n#ifdef VENUS_EXT_shader_framebuffer_fetch\n");
...
bool FGlslCodeBackend::ApplyAndVerifyPlatformRestrictions(exec_list* Instructions, _mesa_glsl_parse_state* ParseState, EHlslShaderFrequency Frequency)
{
	if (ParseState->bGenerateES)
	{
		...
		// ExpandMatricesIntoArrays(Instructions, ParseState);  // haojingyu GLES2 shader去掉矩阵转vec4*4的操作
...
#undef _mesh_glsl_report
	...
	else if (Semantic && strnicmp(Variable->name, "gl_", 3) != 0)
	{
		Variable->semantic = ralloc_strdup(Variable, Semantic);
...
)
{
				...
				&& stricmp(SystemValues[i].Semantic, Semantic) == 0)
			{
				...
				else
				{
					...
					if (SemanticArrayIndex == 0)
					{
						...
						if (stricmp(Semantic, "SV_Position") == 0 && Frequency == HSF_PixelShader)
						{
	...
	{
		const int PrefixLength = 9;
		if (strnicmp(Semantic, "SV_ViewID", PrefixLength) == 0)
		{
	...
	if (Variable == NULL && Frequency == HSF_DomainShader)
	{
		...
		if (strnicmp(Semantic, "SV_TessFactor", PrefixLength) == 0
	...
	if (Variable == NULL && Frequency == HSF_DomainShader)
	{
		...
		if (strnicmp(Semantic, "SV_InsideTessFactor", PrefixLength) == 0
			&& Semantic[PrefixLength] >= '0'
		...
		else if (stricmp(Semantic, "SV_InsideTessFactor") == 0)
		{
			Variable = new(ParseState)ir_variable(
				Type,
...
{
	{
		...
		for (int i = 0; SystemValues[i].Semantic != NULL; ++i)
		{
			if (stricmp(SystemValues[i].GlslName, "gl_FragCoord") == 0)
			{
