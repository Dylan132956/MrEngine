...
namespace moonriver
{
    ...
    static mad_flow mp3_output(void* data, mad_header const* header, mad_pcm* pcm)
    {
        ...
        clip->m_mutex.lock();
        clip->m_stream_buffers.push_back(buffer);
    ...
    Ref<AudioClip> AudioClip::LoadWaveFromFile(const std::string& path)
    {
        Ref<AudioClip> clip;
        ...
        {
            Log("wav file not exist: %s", path.c_str());
        }
...
#if !VR_WASM
    Ref<AudioClip> AudioClip::LoadMp3FromFile(const std::string& path)
    {
        Ref<AudioClip> clip;
        ...
        {
            Log("mp3 file not exist: %s", path.c_str());
        }
...
#if !VR_WASM
    std::vector<void*> AudioClip::GetStreamBuffers()
    {
        ...
        {
            buffers.push_back((void*) (size_t) i);
        }
