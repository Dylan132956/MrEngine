...
#define MOONRIVER_REFLECTION_NEW(name, ...) Reflection::ReflectionPtr(#name, new name(__VA_ARGS__));
#define MOONRIVER_REFLECTION_DELETE(value) \
    if (value) \
    { \
...
#define TypeMetaDef(class_name, ptr) \
    moonriver::Reflection::ReflectionInstance(moonriver::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                            (class_name*)ptr)

#define TypeMetaDefPtr(class_name, ptr) \
    new moonriver::Reflection::ReflectionInstance(moonriver::Reflection::TypeMeta::newMetaFromName(#class_name), \
                                                (class_name*)ptr)
    ...
    namespace Reflection
    {
        ...
        public:
			...
			ReflectionInstance(const ReflectionInstance& str)
			{
				if (this == &str)
				{
					return;
				}
				m_instance = str.m_instance;
				m_meta = str.m_meta;
			}
